...
//        发音人,默认为”xiaoyan”,可以设置的参数列表可参考“合成发音人列表” [_iFlySpeechSynthesizer setParameter:@" xiaoyan " forKey: [IFlySpeechConstant VOICE_NAME]]; //保存合成文件名,如不再需要,设置设置为nil或者为空表示取消,默认目录位于 library/cache下
        iFlySpeechSynthesizer!.setParameter(XJUserDefault.sharedInstance.VoicePeople(), forKey: IFlySpeechConstant.VOICE_NAME())
        ...
        cancelButton.addTarget(self, action: #selector(XTVoiceServiceController.cancelVoiceProcess), forControlEvents: .TouchUpInside)
